...
func (uq *UserQuery) QueryRequisitions() *RequisitionQuery {
	query := &RequisitionQuery{config: uq.config}
...
func (uq *UserQuery) WithRequisitions(opts ...func(*RequisitionQuery)) *UserQuery {
	query := &RequisitionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withRequisitions = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withRequisitions; query != nil {
		...
		query.Where(predicate.Requisition(func(s *sql.Selector) {
			s.Where(sql.InValues(user.RequisitionsColumn, fks...))
		}))
